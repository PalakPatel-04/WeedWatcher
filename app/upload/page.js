...
const UploadPage = () => {
  ...
  const detectImage = async () => {
    ...
    try {
      ...
      const config = {
        ...
        headers: {
          "Api-Key": "DDc7V56G4vg46hfxQF3MbTRvKWvIyygjZeyt4qYWx1zqBOsj5m",
      ...
      const topPlant = plantResults[0].name;
      const SESSION_ID = response.data.access_token;
  ...
  const getPlantInfo = async (plantName, SESSION_ID) => {
    try {
      ...
      const config = {
        method: "post",
        url: `https://plant.id/api/v3/identification/${SESSION_ID}/conversation`,
        headers: {
          "Api-Key": "DDc7V56G4vg46hfxQF3MbTRvKWvIyygjZeyt4qYWx1zqBOsj5m",
          "Content-Type": "application/json"
        },
      ...
      const response = await axios(config);
      return response.data.messages[1].content || "No additional info available.";
    } catch (error) {
  ...
  return (
    <>
      <div className="relative min-h-screen flex flex-col items-center justify-between bg-black">
        ...
        <div className="flex-grow flex flex-col items-center justify-center px-4 pt-16 pb-12 w-full">
          <div className="w-full max-w-md flex flex-col items-center space-y-6">
            {loading ? (
              <div className="w-full h-64 flex items-center justify-center">
                <div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-blue-500"></div>
                <p className="text-white ml-4">Uploading...</p>
              </div>
            ) : (
              <div className="w-full rounded-xl overflow-hidden shadow-lg border border-gray-500 bg-black relative">
                {photo ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-6 bg-gray-900 text-white rounded-xl border border-gray-600 shadow-lg">
                    <Upload fontSize="large" className="text-white mb-4 mt-4" />
                  </div>
                )}
              </div>
            )}
            {!photo && (
              <label htmlFor="upload-input">
                <Button
                  ...
                  component="span"
                  startIcon={<Upload />}
                  className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-xl shadow-md"
                >Upload Image
                </Button>
              </label>
            )}
          ...
          {photo && (
            <div className="mt-8 flex justify-center items-center gap-6 w-full max-w-md">
              <Button
                variant="contained"
                onClick={retakePhoto}
                startIcon={<Replay />}
                className="bg-gray-700 hover:bg-gray-600 text-white px-6 py-3 rounded-xl w-40 h-14 shadow-md"
                disabled={loading}
              >Retake
              </Button>
              <Button
                ...
                onClick={detectImage}
                startIcon={<Search />}
                className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl w-40 h-14 shadow-md"
                disabled={loading}
              >
