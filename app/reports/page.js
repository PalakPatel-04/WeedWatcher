  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        // Set the sorted reports to state, ensuring no duplicates
                ...
                <div key={report.id || index} className={`rounded-lg overflow-hidden shadow-md ${theme === "dark" ? "text-white bg-gray-800" : "text-black bg-white"}`}>
                  ...
                  <div className="p-4">
                    ...
                    <Link href={`/reports/${report._id}`} className="block w-full">
                      <button className="w-full bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded transition-colors">View Details</button>
                    </Link>
                  </div>
                </div>
              ))}
